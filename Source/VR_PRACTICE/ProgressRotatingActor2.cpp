...
// 생성자
AProgressRotatingActor2::AProgressRotatingActor2()
{
	PrimaryActorTick.bCanEverTick = true;
}

// BeginPlay: 자동으로 회전 메시 할당
void AProgressRotatingActor2::BeginPlay()
{
	Super::BeginPlay();
	...
	for (USceneComponent* Comp : ChildrenComp)
	{
		...
		{
			if (Mesh->GetName().Contains(TEXT("Rotate_Tool2")))
			{
				RotateComp = Mesh;
...
// Tick: 진행도 기반 회전 로직
void AProgressRotatingActor2::Tick(float DeltaTime)
{
...
// 진행도 수신 함수
void AProgressRotatingActor2::OnProgressUpdate_Rot(float NewProgress)
{
